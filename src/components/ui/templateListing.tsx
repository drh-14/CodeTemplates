...
export default function TemplateListing(props: {templates : {name: string, id: string}[]}){
    return(
        <div className = 'flex flex-col gap-20 items-center'>
            ...
            <div className = "grid [grid-template-columns:repeat(2,200px)] gap-4">
                {props.templates.map(template => <TemplateBox key = {template.id} name = {template.name} id = {template.id}></TemplateBox>)}
            </div>
        </div>
    );
}